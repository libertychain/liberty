    ...
    {   
       ...
       $movie_genres_id = $genres_info->id;
        ...
        if(isset($_GET['filter']))
        {
            ...
            if($keyword=='old')
            {
                $movies_list = Movies::where('status',1)->whereRaw("find_in_set('$movie_genres_id',movie_genre_id)")->orderBy('id','ASC')->paginate(12);
                $movies_list->appends(\Request::only('filter'))->links();
            }
            else if($keyword=='alpha')
            {
                $movies_list = Movies::where('status',1)->whereRaw("find_in_set('$movie_genres_id',movie_genre_id)")->orderBy('video_title','ASC')->paginate(12);
                $movies_list->appends(\Request::only('filter'))->links();
            }
            else if($keyword=='rand')
            {
                $movies_list = Movies::where('status',1)->whereRaw("find_in_set('$movie_genres_id',movie_genre_id)")->inRandomOrder()->paginate(12);
                $movies_list->appends(\Request::only('filter'))->links();
            }
            else
            {
                $movies_list = Movies::where('status',1)->whereRaw("find_in_set('$movie_genres_id',movie_genre_id)")->orderBy('id','DESC')->paginate(12);
                $movies_list->appends(\Request::only('filter'))->links();
            }
        ...
        else
        {
         ...
         $movies_list = Movies::where('status',1)->whereRaw("find_in_set('$movie_genres_id',movie_genre_id)")->paginate(12);
        }
